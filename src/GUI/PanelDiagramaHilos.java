...
public class PanelDiagramaHilos extends JPanel implements Runnable {
    ...
    private final Map<String, ArrayDeque<Estado>> historialPorHilo = new LinkedHashMap<>();
    ...
    public synchronized void actualizarEstado(String nombreHilo, Estado estado) {
        historialPorHilo.putIfAbsent(nombreHilo, new ArrayDeque<>());
        ArrayDeque<Estado> fila = historialPorHilo.get(nombreHilo);
        if (fila.size() == MAX_TICKS) {
            fila.removeFirst();
        }
        fila.addLast(estado);
        repaint();
    ...
    protected void paintComponent(Graphics g) {
        ...
        int fila = 1;
        for (Map.Entry<String, ArrayDeque<Estado>> entry : historialPorHilo.entrySet()) {
            String nombre = entry.getKey();
            ArrayDeque<Estado> historial = entry.getValue();
